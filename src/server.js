...
async function convertToSGD(amount, currency) {
    // If already SGD, no conversion needed
    if (currency.toUpperCase() === "SGD") {
        return {
            amount_base_currency: Number(amount).toFixed(2),
            base_currency: "SGD",
        };
    }
    ...
    try {
        ...
        const rateObj = data[currencyCode];

        if (!rateObj || !rateObj.sgd) {
            throw new Error(`No exchange rate found for ${currency} to SGD`);
        }

        const exchangeRate = rateObj.sgd;
        const amountInSGD = (Number(amount) * exchangeRate).toFixed(2);

        return {
            amount_base_currency: amountInSGD,
            base_currency: "SGD",
        };
    } catch (error) {
        console.error(`Currency conversion error: ${error.message}`);
        // Fall back to storing original currency as base for failed conversions
        return {
            amount_base_currency: Number(amount).toFixed(2),
    ...
    try {
        ...
        const categoriesResult = await pool.query(
            `SELECT
                ...
                COALESCE(SUM(t.amount_base_currency), 0)::numeric(12,2) AS amount
             FROM transactions t
             LEFT JOIN mcc_reference mr ON mr.mcc = t.mcc_code
             ${whereSql}
             GROUP BY 1
...
app.get("/api/overview/timeseries", async (req, res) => {
    try {
        ...
        const parsed = schema.parse(req.query);
        const where = [];
        const values = [parsed.granularity, parsed.timezone];
        ...
        const { rows } = await pool.query(
            `SELECT
                CASE
                    WHEN $1 = 'year' THEN to_char(date_trunc('year', t.transaction_timestamp AT TIME ZONE $2), 'YYYY')
                    WHEN $1 = 'month' THEN to_char(date_trunc('month', t.transaction_timestamp AT TIME ZONE $2), 'YYYY-MM')
                    ELSE to_char(date_trunc('day', t.transaction_timestamp AT TIME ZONE $2), 'YYYY-MM-DD')
                END AS period_key,
                CASE
                    WHEN $1 = 'year' THEN to_char(date_trunc('year', t.transaction_timestamp AT TIME ZONE $2), 'YYYY')
                    WHEN $1 = 'month' THEN to_char(date_trunc('month', t.transaction_timestamp AT TIME ZONE $2), 'Mon YYYY')
                    ELSE to_char(date_trunc('day', t.transaction_timestamp AT TIME ZONE $2), 'DD Mon YYYY')
                END AS period_label,
                COALESCE(SUM(t.amount_base_currency), 0)::numeric(12,2) AS total,
                CASE
                    WHEN $1 = 'year' THEN date_trunc('year', t.transaction_timestamp AT TIME ZONE $2)
                    WHEN $1 = 'month' THEN date_trunc('month', t.transaction_timestamp AT TIME ZONE $2)
                    ELSE date_trunc('day', t.transaction_timestamp AT TIME ZONE $2)
                END AS period_start
             FROM transactions t
             ${whereSql}
...
app.post("/api/transactions", async (req, res) => {
    try {
        ...
        const { amount_base_currency, base_currency } = await convertToSGD(
            body.amount,
            body.currency
        );
        ...
        const { rows } = await pool.query(
            ...
            VALUES ($1, $2, $3, UPPER($4), $5, $6, $7, NULLIF($8, ''), $9::timestamptz)
            RETURNING *`,
            [
                body.account_id,
                body.merchant ?? null,
            ...
            const { rows } = await client.query(
                `UPDATE transactions
                 SET merchant = $1,
                     amount = $2,
                     currency = UPPER($3),
                     amount_base_currency = $4,
                     base_currency = $5,
                     category = $6,
                     mcc_code = NULLIF($7, ''),
                     transaction_timestamp = $8::timestamptz,
                     merchant_id = $9
                 WHERE id = $10
                 RETURNING *`,
                [
